                ...
                <tbody>
                    @foreach($labels as $key => $label)
                        <tr data-entry-id="{{ $label->id }}">
                            ...
                            <td>
                                @can('label_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.labels.show', $label->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('label_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.labels.edit', $label->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('label_delete')
                                    <form action="{{ route('admin.labels.destroy', $label->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
