...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.openedMail.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.opened-mails.update", [$openedMail->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="email">{{ trans('cruds.openedMail.fields.email') }}</label>
                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email" name="email" id="email" value="{{ old('email', $openedMail->email) }}">
                @if($errors->has('email'))
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('open_time') ? 'is-invalid' : '' }}" type="text" name="open_time" id="open_time" value="{{ old('open_time', $openedMail->open_time) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('open_date') ? 'is-invalid' : '' }}" type="text" name="open_date" id="open_date" value="{{ old('open_date', $openedMail->open_date) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('campaige') ? 'is-invalid' : '' }}" name="campaige_id" id="campaige_id">
                    @foreach($campaiges as $id => $entry)
                        <option value="{{ $id }}" {{ (old('campaige_id') ? old('campaige_id') : $openedMail->campaige->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
