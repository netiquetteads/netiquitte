            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('payment_method_type_id') ? 'is-invalid' : '' }}" name="payment_method_type_id" id="payment_method_type_id">
                    ...
                    @foreach($paymentMethodTypes as $key => $paymentMethodType)
                        <option value="{{ $paymentMethodType->id }}" {{ old('payment_method_type_id', $paymentMethod->payment_method_type_id) === $paymentMethodType->id ? 'selected' : '' }}>{{ $paymentMethodType->name }}</option>
                    @endforeach
                </select>
                @if($errors->has('name'))
                    <span class="text-danger">{{ $errors->first('name') }}</span>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('affiliate_id') ? 'is-invalid' : '' }}" name="affiliate_id" id="affiliate_id">
                    ...
                    @foreach($affiliates as $key => $affiliate)
                        <option value="{{ $affiliate->id }}" {{ old('affiliate_id', $paymentMethod->affiliate_id) === $affiliate->id ? 'selected' : '' }}>{{ $affiliate->name }}</option>
                    @endforeach
                </select>
                @if($errors->has('name'))
                    <span class="text-danger">{{ $errors->first('name') }}</span>
