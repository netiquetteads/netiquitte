...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.mailHistory.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.mail-histories.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('posted_campaign') ? 'is-invalid' : '' }}" name="posted_campaign_id" id="posted_campaign_id">
                    @foreach($posted_campaigns as $id => $entry)
                        <option value="{{ $id }}" {{ old('posted_campaign_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('campaign') ? 'is-invalid' : '' }}" type="text" name="campaign" id="campaign" value="{{ old('campaign', '') }}">
