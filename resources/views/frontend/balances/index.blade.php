        ...
        <div class="col-md-12">
            @can('balance_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.balances.create') }}">
                            ...
                            <tbody>
                                @foreach($balances as $key => $balance)
                                    <tr data-entry-id="{{ $balance->id }}">
                                        ...
                                        <td>
                                            @can('balance_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.balances.show', $balance->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('balance_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.balances.edit', $balance->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('balance_delete')
                                                <form action="{{ route('frontend.balances.destroy', $balance->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('balance_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.balances.massDestroy') }}",
